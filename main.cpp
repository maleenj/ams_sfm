...
public:
    ...
    void runSfM() {
        ...
        if (saveCloud != "") {
            ...
            //cout << "Save point cloud to: " + saveCloud;
            spdlog::info("Save point cloud to: " + saveCloud);
...
private:
    void findImagesInDiretcory(const string& dir) {
        ...
        spdlog::info("Finding images in " + dir);
       ...
       // CV_LOG_INFO(TAG, "Found " + std::to_string(imagesFilenames.size()) + " images");
        spdlog::info("Found " + std::to_string(imagesFilenames.size()) + " images");
    ...
    void extractFeatures() {
        ...
        for (const auto& i : imagesFilenames) {
            ...
            spdlog::info("Found " + to_string(keypoints[i].size()) + " keypoints in " + i);

            spdlog::info("yei"+to_string(saveDebugVisualizations));
            ...
            for (size_t j = i + 1; j < imagesFilenames.size(); ++j) {
                ...
                if ((float)final.size() / (float)match.size() < PAIR_MATCH_SURVIVAL_RATE) {
                   // CV_LOG_INFO(TAG, "Final match '" + imgi + "'->'" + imgj + "' has less than "+to_string(PAIR_MATCH_SURVIVAL_RATE)+" inliers from orignal. Skip");
                    spdlog::info("Final match '" + imgi + "'->'" + imgj + "' has less than "+to_string(PAIR_MATCH_SURVIVAL_RATE)+" inliers from orignal. Skip");
                    continue;
                }
               ...
               // CV_LOG_INFO(TAG, "Matching " + imgi + " and " + imgj + ": " + to_string(final.size()) + " / " + to_string(match.size()));
                spdlog::info("Matching " + imgi + " and " + imgj + ": " + to_string(final.size()) + " / " + to_string(match.size()));
                if (saveDebugVisualizations) {
                    Mat out;
    ...
    void buildTracks() {
       ...
       // CV_LOG_INFO(TAG, "Number of good components: " + to_string(goodComponents.size()));
        spdlog::info("Total number of components found: " + to_string(components.size()));
        spdlog::info("Number of good components: " + to_string(goodComponents.size()));
       ...
       // CV_LOG_INFO(TAG, "Average component size: " + to_string((float)accum / (float)(goodComponents.size())));
        spdlog::info("Average component size: " + to_string((float)accum / (float)(goodComponents.size())));
        if (saveDebugVisualizations) {
    ...
    bool reconstructFromTracks() {
       // CV_LOG_INFO(TAG, "Reconstruct from " + to_string(tracks[0].cols) + " tracks");
        spdlog::info("Reconstruct from " + to_string(tracks[0].cols) + " tracks");
       ...
       // CV_LOG_INFO(TAG, K_);
        ...
        spdlog::info("Reconstruction: ");
        spdlog::info("Estimated 3D points: " + to_string(points3d.size()));
        spdlog::info("Estimated cameras: " + to_string(Rs.size()));
        //spdlog::info("Refined intrinsics: ");
        //spdlog::info(to_string(K_));

        if (Rs.size() != imagesFilenames.size()) {
         //   CV_LOG_ERROR(TAG, "Unable to reconstruct all camera views (" + to_string(imagesFilenames.size()) + ")");
            spdlog::error("Unable to reconstruct all camera views (" + to_string(imagesFilenames.size()) + ")");
            return false;
        }

        if (tracks[0].cols != points3d.size()) {
            spdlog::warn("Unable to reconstruct all tracks (" + to_string(tracks[0].cols) + ")");
    ...
    void visualize3D() {
      //  CV_LOG_INFO(TAG, "Visualize reconstruction");
        spdlog::info("Visualize reconstruction");
    ...
    void saveToMVSFile() {
       ...
       spdlog::info("Save reconstruction to MVS file: " + saveMVS);
    ...
    const bool saveDebugVisualizations=true;       // Save debug visualizations from the reconstruction process
